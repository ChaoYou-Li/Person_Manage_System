...
public class FamilyInfoController {
    ...
    @GetMapping("/list")
    public String findAllFamily(@RequestParam(value = "currentPage", defaultValue = "0") Integer currentPage, @RequestParam(value = "limit", defaultValue = "3") Integer limit, Map<String, Object> map){
        List<FamilyInfo> list = familyInfoService.findFamilyInfoAll(currentPage, limit);
        Integer allCount = familyInfoService.findFamilyInfoAllCount();
        map.put("currentPage", currentPage);
        map.put("totalPage", allCount);
        map.put("list", list);
        map.put("URL", "/FamilyInfo");
        map.put("operation", "family_info_model");
        return "employee/employee_list";
    }


    @GetMapping("/create")
    public String goFamilyCreate(Map<String, Object> map){
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        System.out.println(list);
        map.put("employees", list);
        map.put("type", "create");
        map.put("operation", "family_info_model");
        map.put("req_url", "/FamilyInfo/add");
        return "employee/employee_create";
    }

    @GetMapping("add")
    public String addFamily(FamilyInfo familyInfo, Map<String, Object> map){
    ...
    public String goUpdateFamily(@PathVariable("id") Integer id, Map<String, Object> map){
        FamilyInfo familyInfo = familyInfoService.findFamilyInfo(id);
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        map.put("familyInfo", familyInfo);
        map.put("type", "update");
        map.put("operation", "family_info_model");
        map.put("req_url", "/FamilyInfo/update");
        map.put("employees", list);
        return "employee/employee_create";
    }

    @PostMapping("/update")
    public String updateFamily(FamilyInfo familyInfo, Map<String, Object> map){
        System.out.println(familyInfo);
        FamilyInfo familyInfo1 = familyInfoService.findFamilyInfo(familyInfo.getFamId());
        if (familyInfo1 != null){
    ...
    public String deleteFamily(@PathVariable("id") Integer id, Map<String, Object> map){
        FamilyInfo familyInfo = familyInfoService.findFamilyInfo(id);
        if (familyInfo != null){
    ...
    public String batchDeleteBirth(@RequestParam("data_id") String data_id, Map<String, Object> map){
        ...
        for (int i = 0; i < id.length; i++){
            FamilyInfo familyInfo = familyInfoService.findFamilyInfo(Integer.parseInt(id[i]));
            if (familyInfo != null){
                familyInfoService.deleteFamilyInfo(Integer.parseInt(id[i]));
            }
        }
        return "result/success";
    }
    ...
    public String searchBirthRecord(@RequestParam("data") String data, Map<String, Object> map){
        List<FamilyInfo> list = familyInfoService.findAllFamilyInfoBySearch(data);
        map.put("list", list);
        map.put("operation", "family_info_model");
        return "employee/employee_list";
    }
    ...
    public JSONObject updateExcel(@RequestParam("file") MultipartFile file, UploadUtils uploadUtils) throws Exception {
        ...
        try {
            ...
            for (String[] str:list){
                ...
                ExcelValuesHelperUtils.setAttributeValue(familyInfo, str);
                 ...
                 */
                if (familyInfoService.findFamilyInfo(Integer.parseInt(str[0])) != null){
                    familyInfoService.updateFamilyInfo(familyInfo);
                } else {
                    familyInfoService.insertFamilyInfo(familyInfo);
                }
    ...
    public void exportExcel(HttpServletResponse response) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        //先把数据库中的数据查询出来
        List<FamilyInfo> list1 = familyInfoService.selectList(null);
        //一个设置Excel表标题信息的工具类，获取Excel表标题的字符串数组
        String[] strings = ExcelTitlesHelperUtils.getFamilyInfoTitles();
