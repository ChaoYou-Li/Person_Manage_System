...
public class FreeChargeController {
    ...
    @GetMapping("/list")
    public String findFreeCharge(@RequestParam(value = "currentPage", defaultValue = "0") Integer currentPage, @RequestParam(value = "limit", defaultValue = "3") Integer limit, Map<String, Object> map){
        List<FreeCharge> list = freeChargeService.findFreeChargeAll(currentPage, limit);
        Integer addCount = freeChargeService.findFreeChargeAllCount();
        map.put("currentPage", currentPage);
        map.put("list", list);
        map.put("totalPage", addCount);
        map.put("operation", "free_charge_model");
        map.put("URL", "/FreeCharge");
        return "employee/employee_list";
    }

    @GetMapping("/create")
    public String goFreeCreate(Map<String, Object> map){
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        map.put("employees", list);
        map.put("type", "create");
        map.put("operation", "free_charge_model");
        map.put("req_url", "/FreeCharge/add");
        return "employee/employee_create";
    }

    @GetMapping("/add")
    public String addFreeCharge(FreeCharge freeCharge, Map<String, Object> map){
        freeChargeService.insertFreeCharge(freeCharge);
        map.put("operat", "success");
        return "result/success";
    }

    @GetMapping("/update/{id}")
    public String goUpdateFree(@PathVariable("id") Integer id, Map<String, Object> map){
        FreeCharge freeCharge = freeChargeService.findFreeCharge(id);
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        map.put("employees", list);
        map.put("type", "update");
        map.put("operation", "free_charge_model");
        map.put("req_url", "/FreeCharge/update");
        map.put("freeCharge", freeCharge);
        return "employee/employee_create";
    }

    @PostMapping("/update")
    public String updateFree(FreeCharge freeCharge, Map<String, Object> map){
        FreeCharge freeCharge1 = freeChargeService.findFreeCharge(freeCharge.getFreeId());
        if (freeCharge1 != null){
            freeChargeService.updateFreeCharge(freeCharge);
        }
        return "result/success";
    }

    @DeleteMapping("/delete/{id}")
    public String deleteFreeCharge(@PathVariable("id") Integer id, Map<String, Object> map){
        FreeCharge freeCharge = freeChargeService.findFreeCharge(id);
        if (freeCharge != null){
            freeChargeService.deleteFreeCharge(id);
        }
        return "result/success";
    }
    ...
    public String batchDeleteBirth(@RequestParam("data_id") String data_id, Map<String, Object> map){
        ...
        for (int i = 0; i < id.length; i++){
            FreeCharge freeCharge = freeChargeService.findFreeCharge(Integer.parseInt(id[i]));
            if (freeCharge != null){
                freeChargeService.deleteFreeCharge(Integer.parseInt(id[i]));
            }
        }
        return "result/success";
    }
    ...
    public String searchBirthRecord(@RequestParam("data") String data, Map<String, Object> map){
        List<FreeCharge> list = freeChargeService.findAllFreeChargeBySearch(data);
        map.put("list", list);
        map.put("operation", "free_charge_model");
        return "employee/employee_list";
    }
    ...
    public JSONObject updateExcel(@RequestParam("file") MultipartFile file, UploadUtils uploadUtils) throws Exception {
        ...
        try {
            ...
            for (String[] str:list){
                ...
                ExcelValuesHelperUtils.setAttributeValue(freeCharge, str);
                 ...
                 */
                if (freeChargeService.findFreeCharge(Integer.parseInt(str[0])) != null){
                    freeChargeService.updateFreeCharge(freeCharge);
                } else {
                    freeChargeService.insertFreeCharge(freeCharge);
                }
    ...
    public void exportExcel(HttpServletResponse response) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        //先把数据库中的数据查询出来
        List<FreeCharge> list1 = freeChargeService.selectList(null);
        //一个设置Excel表标题信息的工具类，获取Excel表标题的字符串数组
        String[] strings = ExcelTitlesHelperUtils.getFreeChargeTitles();
