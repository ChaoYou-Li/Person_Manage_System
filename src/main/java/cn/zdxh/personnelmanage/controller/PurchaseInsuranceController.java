...
public class PurchaseInsuranceController {
    ...
    @Autowired
    private PurchaseInsuranceService purchaseInsuranceService;

    @GetMapping("/list")
    public String findPurchaseInsurance(@RequestParam(value = "currentPage", defaultValue = "0") Integer currentPage, @RequestParam(value = "limit", defaultValue = "3") Integer limit, Map<String, Object> map){
        List<PurchaseInsurance> list = purchaseInsuranceService.findAllPurchaseInsurance(currentPage, limit);
        Integer addCount = purchaseInsuranceService.findPurchaseInsuranceAllCount();
        map.put("list", list);
        map.put("currentPage", currentPage);
        map.put("totalPage", addCount);
        map.put("operation", "purchase_insurance_model");
        map.put("URL", "/PurchaseInsurance");
        return "employee/employee_list";
    }

    @GetMapping("/create")
    public String goPurchaseCreate(Map<String, Object> map){
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        map.put("type", "create");
        map.put("operation", "purchase_insurance_model");
        map.put("req_url", "/PurchaseInsurance/add");
        map.put("employees", list);
        return "employee/employee_create";
    }

    @GetMapping("/add")
    public String addPurchaseInsurance(PurchaseInsurance purchaseInsurance, Map<String, Object> map){
        if (purchaseInsurance != null){
            purchaseInsuranceService.insertPurchaseInsurance(purchaseInsurance);
        }
    ...
    public String goUpdatePurchaseInsurance(@PathVariable("id") Integer id, Map<String, Object> map){
        PurchaseInsurance purchaseInsurance = purchaseInsuranceService.findPurchaseInsurance(id);
        List<EmployeeInfo> list = employeeInfoService.selectList(null);
        map.put("type", "update");
        map.put("operation", "purchase_insurance_model");
        map.put("req_url", "/PurchaseInsurance/update");
        map.put("employees", list);
        map.put("purchaseInsurance", purchaseInsurance);
        return "employee/employee_create";
    }

    @PostMapping("/update")
    public String updatePurchaseInsurance(PurchaseInsurance purchaseInsurance, Map<String, Object> map){
        PurchaseInsurance purchaseInsurance1 = purchaseInsuranceService.findPurchaseInsurance(purchaseInsurance.getPurId());
        if (purchaseInsurance1 != null){
            purchaseInsuranceService.updatePurchaseInsurance(purchaseInsurance);
        }
    ...
    public String deletePurchaseInsurance(@PathVariable("id") Integer id, Map<String, Object> map){
        PurchaseInsurance purchaseInsurance = purchaseInsuranceService.findPurchaseInsurance(id);
        if (purchaseInsurance != null){
            purchaseInsuranceService.deletePurchaseInsurance(id);
        }
        map.put("operat", "success");
        return "result/success";
    }
    ...
    public String batchDeleteBirth(@RequestParam("data_id") String data_id, Map<String, Object> map){
        ...
        for (int i = 0; i < id.length; i++){
            PurchaseInsurance purchaseInsurance = purchaseInsuranceService.findPurchaseInsurance(Integer.parseInt(id[i]));
            if (purchaseInsurance != null){
                purchaseInsuranceService.deletePurchaseInsurance(Integer.parseInt(id[i]));
            }
        }
        return "result/success";
    }
    ...
    public String searchBirthRecord(@RequestParam("data") String data, Map<String, Object> map){
        List<PurchaseInsurance> list = purchaseInsuranceService.findAllPurchaseInsuranceBySearch(data);
        map.put("list", list);
        map.put("operation", "purchase_insurance_model");
        return "employee/employee_list";
    }
    ...
    public JSONObject updateExcel(@RequestParam("file") MultipartFile file, UploadUtils uploadUtils) throws Exception {
        ...
        try {
            ...
            for (String[] str:list){
                ...
                PurchaseInsurance professionalSkills = new PurchaseInsurance();
                 ...
                 */
                if (purchaseInsuranceService.findPurchaseInsurance(Integer.parseInt(str[0])) != null){
                    purchaseInsuranceService.updatePurchaseInsurance(professionalSkills);
                } else {
                    purchaseInsuranceService.insertPurchaseInsurance(professionalSkills);
                }
            }
        } catch (Exception e){
            /**
    ...
    public void exportExcel(HttpServletResponse response) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        //先把数据库中的数据查询出来
        List<PurchaseInsurance> list1 = purchaseInsuranceService.selectList(null);
        //一个设置Excel表标题信息的工具类，获取Excel表标题的字符串数组
        String[] strings = ExcelTitlesHelperUtils.getPurchaseInsuranceTitles();
